...
export default function AuthPage({ defaultMode = "login" }) {
  ...
  const handleAuth = async (e) => {
    ...
    if (mode === "signup") {
      ...
      try {
        ...
        if (role === "founder") {
          navigate("/founder-dashboard", {
            state: { name: res.user.name, company: res.user.company }
          });
        } else {
          navigate("/investor-dashboard", {
            state: { name: res.user.name, fund: res.user.company }
          });
        }
      } catch (err) {
      ...
      // Login flow
      try {
        const res = await apiService.login(email, pass, role);
        setError("");
        if (role === "founder") {
          navigate("/founder-dashboard", {
            state: { name: res.user.name, company: res.user.company }
        ...
        setError(err.response?.data?.error || "Invalid credentials or role selection.");
      }
    }
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleAuth} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: mutedCol, marginBottom: 8, letterSpacing: "0.8px" }}>
              {mode === "login" ? "LOGIN AS" : "SELECT ROLE"}
            </label>
